<%@ Page Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="WorkerCategorySetup.aspx.cs" Inherits="HRMS.WorkerCategorySetupPage" %>
<asp:Content ID="Main" ContentPlaceHolderID="MainContent" runat="server">
<% if (!string.IsNullOrEmpty(AlertMessage)) { %><div class="alert alert-<%= AlertType %>"><%= Server.HtmlEncode(AlertMessage) %></div><% } %>
<div class="card"><div class="card-header"><h2><%= Input.Id > 0 ? "Edit " + ItemLabel : "Add " + ItemLabel %></h2></div>
<div class="card-body">
...
<a href="/<%= PagePath %>.aspx" class="btn btn-secondary">Clear</a></div></div>
<div class="card mt-4"><div class="card-header"><h2>List</h2></div><div class="card-body table-responsive"><table class="data-table">
<thead><tr><th>Code</th><th>Name</th><th>Alias</th><th>Description</th><th>Status</th><th></th></tr></thead><tbody>
<% foreach (var item in Records) { %><tr>
<td><%= Server.HtmlEncode(item.Code) %></td><td><%= Server.HtmlEncode(item.Name) %></td><td><%= Server.HtmlEncode(item.AliasName) %></td><td><%= Server.HtmlEncode(item.Extra) %></td><td><%= item.IsActive ? "Active" : "Inactive" %></td>
