...
namespace HRMS
{
    ...
    public partial class VendorMasterPage : AppBasePage
    {
        ...
        public string PageTitle => "Vendor Master";
        public List<VendorListItem> Vendors { get; set; } = new List<VendorListItem>();
        public VendorInput Input { get; set; } = new VendorInput();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            var newVendor = Request.QueryString["newVendor"] == "1" || Request.QueryString["newVendor"] == "true";
            OnGet(QueryInt("editId"), newVendor);
        }

        private void OnGet(int? editId, bool newVendor)
        {
            LoadAlert(out var msg, out var typ);
            AlertMessage = msg;
            AlertType = typ;
            ShowForm = (editId.HasValue && editId > 0) || newVendor;
            ...
            if (ShowForm)
            {
                ...
                {
                    Input.VendorCode = GenerateNextVendorCode();
                }
            }
            else
            {
        ...
        private void OnPostSave()
        {
            ...
            {
                SetFormError("Vendor name must be alphanumeric (letters, numbers, spaces, - _ . & allowed).");
                return;
            }
            ...
            try
            {
                ...
                SetAlert(EditMode ? "Vendor updated successfully." : "Vendor created successfully.");
                Response.Redirect("~/VendorMaster.aspx?editId=" + Input.VendorID);
            }
            catch (SqlException ex) when (ex.Number == 2627 || ex.Number == 2601)
            {
                SetFormError("A vendor with this ID or duplicate value already exists.");
            }
            catch (Exception ex)
            {
...
UPDATE tblVendor SET IsActive = 0, ModifiedOn = GETDATE(), ModifiedByUserID = @ModifiedByUserID
WHERE VendorID = @VendorID;", conn))
                {
                    cmd.Parameters.AddWithValue("@VendorID", deleteId);
                    AuditHelper.AddModifiedBy(cmd, Auth.CurrentUserId);
                    conn.Open();
                    cmd.ExecuteNonQuery();
                }
                SetAlert("Vendor removed successfully.");
            }
            catch (Exception ex)
            {
                SetAlert("Error removing vendor: " + ex.Message, "error");
            }
        ...
        private void SaveRecord(SqlConnection conn, VendorInput input, bool editMode)
        {
            if (editMode && input.VendorID > 0)
            {
                using (var cmd = new SqlCommand(@"
UPDATE tblVendor SET Name=@Name, SearchName=@SearchName, DealForBranchID=@DealForBranchID,
  ...
  ModifiedOn=GETDATE(), ModifiedByUserID=@ModifiedByUserID WHERE VendorID=@VendorID;", conn))
                {
                    BindParams(cmd, input);
                    cmd.Parameters.AddWithValue("@VendorID", input.VendorID);
                    AuditHelper.AddModifiedBy(cmd, Auth.CurrentUserId);
                    cmd.ExecuteNonQuery();
                }
                return;
            ...
            input.VendorCode = GenerateNextVendorCode(conn);
            using (var ins = new SqlCommand(@"
INSERT INTO tblVendor
 (VendorCode, Name, SearchName, DealForBranchID, CityID, ProvinceID,
...
SELECT CAST(SCOPE_IDENTITY() AS INT);", conn))
            {
                ...
                ins.Parameters.AddWithValue("@VendorCode", input.VendorCode);
                AuditHelper.AddCreatedBy(ins, Auth.CurrentUserId);
                input.VendorID = (int)ins.ExecuteScalar();
            }
        }

        private static void BindParams(SqlCommand cmd, VendorInput input)
        {
        ...
        private string GenerateNextVendorCode(SqlConnection conn = null)
        {
            var owns = conn == null;
            ...
            {
                using (var cmd = new SqlCommand(@"
SELECT ISNULL(MAX(TRY_CAST(SUBSTRING(VendorCode, 4, 10) AS INT)), 0)
FROM tblVendor WHERE VendorCode LIKE 'VEN[0-9]%';", conn))
                {
                    var next = Convert.ToInt32(cmd.ExecuteScalar()) + 1;
                    return "VEN" + next.ToString("D6");
                }
            }
        ...
        private void LoadVendors()
        {
            ...
            using (var conn = new SqlConnection(Conn))
            using (var cmd = new SqlCommand(@"
SELECT v.VendorID, v.VendorCode, v.Name, ISNULL(v.SearchName, ''),
       ISNULL(ci.CityName, ''), ISNULL(g.CustomerGroupName, ''), ISNULL(v.NTN, ''), v.IsActive
FROM tblVendor v
LEFT JOIN tblCity ci ON ci.CityID = v.CityID
LEFT JOIN tblCustomerGroup g ON g.CustomerGroupID = v.CustomerGroupID
ORDER BY v.IsActive DESC, v.VendorCode;", conn))
            {
                conn.Open();
        ...
        private void LoadForEdit(int vendorId)
        {
            using (var conn = new SqlConnection(Conn))
            using (var cmd = new SqlCommand(@"
SELECT VendorID, VendorCode, Name, SearchName,
...
FROM tblVendor WHERE VendorID = @VendorID;", conn))
            {
